...
Matt Pocock AI Coding Skills 的 **pi-agent** 适配版。
把 `grill-with-docs` → `to-spec` → `to-tickets` → `implement` → `code-review` 串成自动循环，让 AI 自己写代码。

> ⚠ **仅支持 pi-agent**（`~/.pi/agent/skills/` 路径），不兼容 Claude Code / Cursor 等其他 agent。
